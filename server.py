...
from flask import Flask, request, jsonify, send_file, render_template, session, redirect, url_for, send_from_directory, \
    Markup
from flask_wtf import FlaskForm, Form
from flask_wtf.file import FileField, FileRequired, FileAllowed
from kijijiapi import *
from wtforms.fields.html5 import DateField, TimeField
from wtforms import SelectField, SelectMultipleField, TextField, TextAreaField, validators, StringField, SubmitField, \
    FieldList, FormField, BooleanField, IntegerField, widgets
...
class PostForm(FlaskForm):
    ...
    adtitle = TextField(id='adtitle', label='Ad Title',
    ...
    pricetype = SelectField(id='pricetype', label='Price Type',
                            choices=['SPECIFIED_AMOUNT', 'PLEASE_CONTACT', 'SWAP_TRADE', 'FREE'])
                        ...
                        try:
                            ...
                            for rule in user['rules']:
                                ...
                                if 'user:user-conversation' in conversations['user:user-conversations']:
                                    ...
                                    isList = testListInstance(
                                        conversations['user:user-conversations']['user:user-conversation'])

                                    # only 1 conversation if not list
                                    if isList == False:

                                        for key, value in conversations['user:user-conversations'][
                                            'user:user-conversation'].items():
                                            sendMessage = False
                                            unread = False
                                            ...
                                            if key == 'user:num-unread-msg':
                                                ...
                                                if value != '0':
                                                    ...
                                                    conversation = getConversation(kijijiSession, userID, userToken,
                                                                                   conversationID)

                                                    adID = conversation['user:user-conversation']['user:ad-id']
                                                    ownerUserID = conversation['user:user-conversation'][
                                                        'user:ad-owner-id']
                                                    ownerEmail = conversation['user:user-conversation'][
                                                        'user:ad-owner-email']
                                                    ownerName = conversation['user:user-conversation'][
                                                        'user:ad-owner-name']
                                                    replierUserID = conversation['user:user-conversation'][
                                                        'user:ad-replier-id']
                                                    replierEmail = conversation['user:user-conversation'][
                                                        'user:ad-replier-email']
                                                    replierName = conversation['user:user-conversation'][
                                                        'user:ad-replier-name']
                                            ...
                                            for index, message in rule.items():
                                                ...
                                                if (re.search(r"\b{}\b".format(message), content,
                                                              re.IGNORECASE) is not None) and unread == True and senderID != userID and answered == 'false':
                                                    sendMessage = True

                                                if index == 'response' and sendMessage == True:
                                                    reply = message
                                                    finalPayload = createReplyPayload(adID, replyName, replyEmail,
                                                                                      reply, conversationID, direction)
                                                    now = datetime.datetime.now()
                                                    sendReply(kijijiSession, userID, userToken, finalPayload)
                                                    print('Auto replied', direction, 'in conversation', conversationID,
                                                          'at', now)
        ...
        if len(attributes) != 0:
            ...
            for key, value in attributes.items():
                ...
                if value == True or value == 'y':
                    attributesPayload['attr:attributes']['attr:attribute'].append(
                        {'@type': '', '@localized-label': '', '@name': key, 'attr:value': 'true'})

                # If above conditions do not apply, and value is not None, append attribute
                if (value != True or value != 'y') and (value != False or value != 'n') and (
                        value != None and value != ''):
                    attributesPayload['attr:attributes']['attr:attribute'].append(
                        {'@type': '', '@localized-label': '', '@name': key, 'attr:value': value})

                # set xml type variable for Date based attributes
                if 'date' in key:
                    attributesPayload['attr:attributes']['attr:attribute'].append(
                        {'@type': 'DATE', '@localized-label': '', '@name': key, 'attr:value': value + 'T00:00:00Z'})
...
def attributes():
    if 'loggedin' in session:
        ...
        try:
            items = [(x['#text'], x['@localized-label']) for x in
                     attributesFile['ad:ad']['ad:ad-type']['ad:supported-value']]
            ...
            if testListInstance(attributesFile['ad:ad']['attr:attributes']['attr:attribute']) == True:
                ...
                for x in attributesFile['ad:ad']['attr:attributes']['attr:attribute']:
                    ...
                    if testDictInstance(x) == True:
                        ...
                        if x['@deprecated'] == 'false' and x['@write'] != 'unsupported' and '@sub-type' not in x and x[
                            '@type'] == 'ENUM':
                            ...
                            if 'attr:supported-value' in x and testDictInstance(x['attr:supported-value']) == True:
                                print(
                                    'Error 0001: This attribute set is currently unimplemented, please report to developer!')
                            ...
                            if 'attr:supported-value' in x and testListInstance(x['attr:supported-value']) == True:
                                for y in x['attr:supported-value']:
                                    ...
                                    if testListInstance(y) == True:
                                        print(
                                            'Error 0002: This attribute set is currently unimplemented, please report to developer!')
...
def make_choice(choice):
    ...
    for x in attributeData['ad:ad']['attr:dependent-attributes']['attr:dependent-attribute'][
        'attr:dependent-supported-value']:
        try:
